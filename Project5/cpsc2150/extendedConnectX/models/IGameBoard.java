...
public interface IGameBoard {
    ...
    default boolean checkHorizWin(BoardPosition pos, char p){
        ...
        for (int col = pos.getColumn() - 1; col >= 0; col--) {
            thisPos = new BoardPosition(thisPos.getRow(), col);
            if (whatsAtPos(thisPos) == p) {
                count++;
                if (count == numToWin) return true;
            }
    ...
    default boolean checkVertWin(BoardPosition pos, char p){
        ...
        int toWin = getNumToWin();
        int row = getNumRows();

        for (int i = 0; i < row; i++) {
            //if that specific player is found, add to the count and check if it meets
            //the win condition
        ...
        //if we've reached the count of the same character and it matches the win condition
        return count == toWin;
    }
    ...
    default boolean checkDiagWin(BoardPosition pos, char p){
        ...
        for(int i = 0; r + i < getNumRows() && c + i < getNumColumns(); i++){
            if(isPlayerAtPos(current,p)) {
                count++;
                if(count == getNumToWin()) return true;
            }
            else count = 0;
            current = new BoardPosition(current.getRow() + 1, current.getColumn() + 1);
        }
        ...
        count = 0;
        current = new BoardPosition(pos.getRow(), pos.getColumn());
        for(int i = 0; r - i >= 0 && c + i < getNumColumns(); i++){
            if(isPlayerAtPos(current,p)) {
                count++;
                if(count == getNumToWin()) return true;
            }
